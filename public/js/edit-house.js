...
axios.get(`${BASE_URL}/houses/${house_id}`, { headers: {
  ...
  .then(response => {
    ...
    document.querySelector('input[name="dateInitial"]').value = new Date(response.data.dateInitial);
    document.querySelector('input[name="dateFinished"]').value = new Date(response.data.dateFinished);
    console.log();
...
document.getElementById('btn-edit').addEventListener('click', function(event) {
    ...
    const dateFinished = document.querySelector('input[name="dateInitial"]').value;
    const description = document.querySelector('input[name="dateFinished"]').value;
